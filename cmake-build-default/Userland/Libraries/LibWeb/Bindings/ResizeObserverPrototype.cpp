...
ResizeObserverPrototype::ResizeObserverPrototype([[maybe_unused]] JS::GlobalObject& global_object)
    : Object(*global_object.object_prototype())

{
}

ResizeObserverPrototype::~ResizeObserverPrototype()
{
}

void ResizeObserverPrototype::initialize(JS::GlobalObject& global_object)
{
    [[maybe_unused]] auto& vm = this->vm();
    [[maybe_unused]] u8 default_attributes = JS::Attribute::Enumerable | JS::Attribute::Configurable | JS::Attribute::Writable;


    define_native_function("observe", observe, 1, default_attributes);

    define_native_function("unobserve", unobserve, 1, default_attributes);

    define_native_function("disconnect", disconnect, 0, default_attributes);

    Object::initialize(global_object);
}

static JS::ThrowCompletionOr<ResizeObserver::ResizeObserver*> impl_from(JS::VM& vm, JS::GlobalObject& global_object)
{
    ...
    if (!is<ResizeObserverWrapper>(this_object))
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::NotAnObjectOfType, "ResizeObserver::ResizeObserver");

    return &static_cast<ResizeObserverWrapper*>(this_object)->impl();
}

JS_DEFINE_NATIVE_FUNCTION(ResizeObserverPrototype::observe)
{

    auto* impl = TRY(impl_from(vm, global_object));

    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::BadArgCountOne, "observe");

    auto arg0 = vm.argument(0);

    if (!arg0.is_object() || !is<ElementWrapper>(arg0.as_object()))
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::NotAnObjectOfType, "Element");

    auto& target = static_cast<ElementWrapper&>(arg0.as_object()).impl();

    auto arg1 = vm.argument(1);

    if (!arg1.is_nullish() && !arg1.is_object())
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::NotAnObjectOfType, "ResizeObserverOptions");
    ...
    if (arg1.is_nullish()) {
        ...
        box = TRY(arg1.as_object().get("box"));
    }

    ResizeObserverBoxOptions box_value { ResizeObserverBoxOptions::ContentBox };
    auto box_string = TRY(box.to_string(global_object));

    if (box_string == "content-box"sv)
        box_value = ResizeObserverBoxOptions::ContentBox;

    else if (box_string == "border-box"sv)
        box_value = ResizeObserverBoxOptions::BorderBox;

    else if (box_string == "device-pixel-content-box"sv)
        box_value = ResizeObserverBoxOptions::DevicePixelContentBox;

    options.box = box_value;

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(global_object, [&] { return impl->observe(target, options); }));

    return JS::js_undefined();

}

JS_DEFINE_NATIVE_FUNCTION(ResizeObserverPrototype::unobserve)
{

    auto* impl = TRY(impl_from(vm, global_object));

    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::BadArgCountOne, "unobserve");
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(global_object, [&] { return impl->unobserve(target); }));

    return JS::js_undefined();

}

JS_DEFINE_NATIVE_FUNCTION(ResizeObserverPrototype::disconnect)
{

    auto* impl = TRY(impl_from(vm, global_object));

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(global_object, [&] { return impl->disconnect(); }));
