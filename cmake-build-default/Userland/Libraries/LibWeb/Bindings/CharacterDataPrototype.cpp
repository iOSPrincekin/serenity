...
CharacterDataPrototype::CharacterDataPrototype([[maybe_unused]] JS::GlobalObject& global_object)
...
void CharacterDataPrototype::initialize(JS::GlobalObject& global_object)
{
    ...
    define_native_function("substringData", substring_data, 2, default_attributes);

    MUST(unscopable_object->create_data_property("after", JS::Value(true)));

    define_native_function("after", after, 0, default_attributes);

    define_native_function("replaceData", replace_data, 3, default_attributes);
...
static JS::ThrowCompletionOr<DOM::CharacterData*> impl_from(JS::VM& vm, JS::GlobalObject& global_object)
{
    ...
    if (!is<CharacterDataWrapper>(this_object))
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::NotAnObjectOfType, "DOM::CharacterData");

    return &static_cast<CharacterDataWrapper*>(this_object)->impl();
}
...
JS_DEFINE_NATIVE_FUNCTION(CharacterDataPrototype::substring_data)
{
    ...
    if (vm.argument_count() < 2)
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::BadArgCountMany, "substringData", "2");
    ...
    count = TRY(arg1.to_u32(global_object));

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(global_object, [&] { return impl->substring_data(offset, count); }));
...
JS_DEFINE_NATIVE_FUNCTION(CharacterDataPrototype::replace_data)
{
    ...
    if (vm.argument_count() < 3)
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::BadArgCountMany, "replaceData", "3");
    ...
    if (arg2.is_null() && false) {
        ...
        data = TRY(arg2.to_string(global_object));
    }

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(global_object, [&] { return impl->replace_data(offset, count, data); }));
