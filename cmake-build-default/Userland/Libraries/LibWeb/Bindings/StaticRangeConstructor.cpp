...
#if __has_include(<LibWeb/Crypto/StaticRange.h>)
#    include <LibWeb/Crypto/StaticRange.h>
#elif __has_include(<LibWeb/CSS/StaticRange.h>)
#    include <LibWeb/CSS/StaticRange.h>
#elif __has_include(<LibWeb/DOM/StaticRange.h>)
#    include <LibWeb/DOM/StaticRange.h>
#elif __has_include(<LibWeb/Encoding/StaticRange.h>)
#    include <LibWeb/Encoding/StaticRange.h>
#elif __has_include(<LibWeb/Geometry/StaticRange.h>)
#    include <LibWeb/Geometry/StaticRange.h>
#elif __has_include(<LibWeb/HTML/StaticRange.h>)
#    include <LibWeb/HTML/StaticRange.h>
#elif __has_include(<LibWeb/UIEvents/StaticRange.h>)
#    include <LibWeb/UIEvents/StaticRange.h>
#elif __has_include(<LibWeb/HighResolutionTime/StaticRange.h>)
#    include <LibWeb/HighResolutionTime/StaticRange.h>
#elif __has_include(<LibWeb/IntersectionObserver/StaticRange.h>)
#    include <LibWeb/IntersectionObserver/StaticRange.h>
#elif __has_include(<LibWeb/NavigationTiming/StaticRange.h>)
#    include <LibWeb/NavigationTiming/StaticRange.h>
#elif __has_include(<LibWeb/RequestIdleCallback/StaticRange.h>)
#    include <LibWeb/RequestIdleCallback/StaticRange.h>
#elif __has_include(<LibWeb/ResizeObserver/StaticRange.h>)
#    include <LibWeb/ResizeObserver/StaticRange.h>
#elif __has_include(<LibWeb/SVG/StaticRange.h>)
#    include <LibWeb/SVG/StaticRange.h>
#elif __has_include(<LibWeb/Selection/StaticRange.h>)
#    include <LibWeb/Selection/StaticRange.h>
#elif __has_include(<LibWeb/WebSockets/StaticRange.h>)
#    include <LibWeb/WebSockets/StaticRange.h>
#elif __has_include(<LibWeb/XHR/StaticRange.h>)
#    include <LibWeb/XHR/StaticRange.h>
#elif __has_include(<LibWeb/URL/StaticRange.h>)
#    include <LibWeb/URL/StaticRange.h>
#endif
...
StaticRangeConstructor::StaticRangeConstructor(JS::GlobalObject& global_object)
    : NativeFunction(*global_object.function_prototype())
{
}

StaticRangeConstructor::~StaticRangeConstructor()
{
}

JS::ThrowCompletionOr<JS::Value> StaticRangeConstructor::call()
{
    return vm().throw_completion<JS::TypeError>(global_object(), JS::ErrorType::ConstructorWithoutNew, "StaticRange");
}

JS::ThrowCompletionOr<JS::Object*> StaticRangeConstructor::construct(FunctionObject&)
{
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::BadArgCountOne, "StaticRange");

    auto arg0 = vm.argument(0);

    if (!arg0.is_nullish() && !arg0.is_object())
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::NotAnObjectOfType, "StaticRangeInit");

    StaticRangeInit init {};

    JS::Value end_container;
    if (arg0.is_nullish()) {
        end_container = JS::js_undefined();
    } else {
        end_container = TRY(arg0.as_object().get("endContainer"));
    }

    if (end_container.is_undefined())
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::MissingRequiredProperty, "endContainer");

    if (!end_container.is_object() || !is<NodeWrapper>(end_container.as_object()))
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::NotAnObjectOfType, "Node");

    auto& end_container_value = static_cast<NodeWrapper&>(end_container.as_object()).impl();

    init.end_container = end_container_value;

    JS::Value end_offset;
    if (arg0.is_nullish()) {
        end_offset = JS::js_undefined();
    } else {
        end_offset = TRY(arg0.as_object().get("endOffset"));
    }

    if (end_offset.is_undefined())
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::MissingRequiredProperty, "endOffset");

    u32 end_offset_value;

    end_offset_value = TRY(end_offset.to_u32(global_object));

    init.end_offset = end_offset_value;

    JS::Value start_container;
    if (arg0.is_nullish()) {
        start_container = JS::js_undefined();
    } else {
        start_container = TRY(arg0.as_object().get("startContainer"));
    }

    if (start_container.is_undefined())
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::MissingRequiredProperty, "startContainer");

    if (!start_container.is_object() || !is<NodeWrapper>(start_container.as_object()))
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::NotAnObjectOfType, "Node");

    auto& start_container_value = static_cast<NodeWrapper&>(start_container.as_object()).impl();

    init.start_container = start_container_value;

    JS::Value start_offset;
    if (arg0.is_nullish()) {
        start_offset = JS::js_undefined();
    } else {
        start_offset = TRY(arg0.as_object().get("startOffset"));
    }

    if (start_offset.is_undefined())
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::MissingRequiredProperty, "startOffset");

    u32 start_offset_value;

    start_offset_value = TRY(start_offset.to_u32(global_object));

    init.start_offset = start_offset_value;

    auto impl = TRY(throw_dom_exception_if_needed(global_object, [&] { return DOM::StaticRange::create_with_global_object(window, init); }));

    return wrap(global_object, *impl);

}

void StaticRangeConstructor::initialize(JS::GlobalObject& global_object)
{
    auto& vm = this->vm();
    auto& window = static_cast<WindowObject&>(global_object);
    ...
    define_direct_property(vm.names.prototype, &window.ensure_web_prototype<StaticRangePrototype>("StaticRange"), 0);
