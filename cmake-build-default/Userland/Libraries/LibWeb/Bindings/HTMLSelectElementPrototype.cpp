...
HTMLSelectElementPrototype::HTMLSelectElementPrototype([[maybe_unused]] JS::GlobalObject& global_object)
...
void HTMLSelectElementPrototype::initialize(JS::GlobalObject& global_object)
{
    ...
    define_native_accessor("options", options_getter, nullptr, default_attributes);

    define_native_accessor("selectedIndex", selected_index_getter, selected_index_setter, default_attributes);

    define_native_function("add", add, 1, default_attributes);

    Object::initialize(global_object);
}

static JS::ThrowCompletionOr<HTML::HTMLSelectElement*> impl_from(JS::VM& vm, JS::GlobalObject& global_object)
{
    ...
    if (!is<HTMLSelectElementWrapper>(this_object))
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::NotAnObjectOfType, "HTML::HTMLSelectElement");

    return &static_cast<HTMLSelectElementWrapper*>(this_object)->impl();
}
...
JS_DEFINE_NATIVE_FUNCTION(HTMLSelectElementPrototype::options_getter)
{
    ...
    auto retval = TRY(throw_dom_exception_if_needed(global_object, [&] { return impl->options(); }));

    return wrap(global_object, const_cast<HTMLOptionsCollection&>(*retval));
...
JS_DEFINE_NATIVE_FUNCTION(HTMLSelectElementPrototype::selected_index_getter)
{
    ...
    auto retval = TRY(throw_dom_exception_if_needed(global_object, [&] { return impl->selected_index(); }));
...
JS_DEFINE_NATIVE_FUNCTION(HTMLSelectElementPrototype::selected_index_setter)
{
    ...
    cpp_value = TRY(value.to_i32(global_object));

    TRY(throw_dom_exception_if_needed(global_object, [&] { return impl->set_selected_index(cpp_value); }));
...
JS_DEFINE_NATIVE_FUNCTION(HTMLSelectElementPrototype::add)
{
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::BadArgCountOne, "add");

    auto arg0 = vm.argument(0);

    auto arg0_to_variant = [&global_object, &vm](JS::Value arg0) -> JS::ThrowCompletionOr<Variant<NonnullRefPtr<HTMLOptionElement>, NonnullRefPtr<HTMLOptGroupElement>>> {
        ...
        if (arg0.is_object()) {
            ...
            if (is<Wrapper>(arg0_object)) {
                ...
                if (is<HTMLOptionElementWrapper>(arg0_object))
                    return NonnullRefPtr<HTMLOptionElement> { static_cast<HTMLOptionElementWrapper&>(arg0_object).impl() };

                if (is<HTMLOptGroupElementWrapper>(arg0_object))
                    return NonnullRefPtr<HTMLOptGroupElement> { static_cast<HTMLOptGroupElementWrapper&>(arg0_object).impl() };
    ...
    Variant<NonnullRefPtr<HTMLOptionElement>, NonnullRefPtr<HTMLOptGroupElement>> element = TRY(arg0_to_variant(arg0));

    auto arg1 = vm.argument(1);

    auto arg1_to_variant = [&global_object, &vm](JS::Value arg1) -> JS::ThrowCompletionOr<Variant<NonnullRefPtr<HTMLElement>, i32>> {
        // These might be unused.
        (void)global_object;
        (void)vm;
        ...
        if (arg1.is_object()) {
            ...
            if (is<Wrapper>(arg1_object)) {

                if (is<HTMLElementWrapper>(arg1_object))
                    return NonnullRefPtr<HTMLElement> { static_cast<HTMLElementWrapper&>(arg1_object).impl() };
    ...
    arg1_number = TRY(arg1.to_i32(global_object));
    ...
    Optional<Variant<NonnullRefPtr<HTMLElement>, i32>> before;
    if (!arg1.is_undefined())
        before = TRY(arg1_to_variant(arg1));

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(global_object, [&] { return impl->add(element, before); }));
