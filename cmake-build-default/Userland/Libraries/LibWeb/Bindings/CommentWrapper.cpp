...
CommentWrapper* CommentWrapper::create(JS::GlobalObject& global_object, DOM::Comment& impl)
{
    return global_object.heap().allocate<CommentWrapper>(global_object, global_object, impl);
}


CommentWrapper::CommentWrapper(JS::GlobalObject& global_object, DOM::Comment& impl)
    : CharacterDataWrapper(global_object, impl)
{
    set_prototype(&static_cast<WindowObject&>(global_object).ensure_web_prototype<CommentPrototype>("Comment"));
}

void CommentWrapper::initialize(JS::GlobalObject& global_object)
{
    CharacterDataWrapper::initialize(global_object);

    auto& vm = global_object.vm();
    define_direct_property(*vm.well_known_symbol_to_string_tag(), JS::js_string(vm, "Comment"), JS::Attribute::Configurable);
}
...
CommentWrapper* wrap(JS::GlobalObject& global_object, DOM::Comment& impl)
{
    return static_cast<CommentWrapper*>(wrap_impl(global_object, impl));
}
