...
SVGCircleElementPrototype::SVGCircleElementPrototype([[maybe_unused]] JS::GlobalObject& global_object)
    : Object(static_cast<WindowObject&>(global_object).ensure_web_prototype<SVGGeometryElementPrototype>("SVGGeometryElement"))

{
}

SVGCircleElementPrototype::~SVGCircleElementPrototype()
{
}

void SVGCircleElementPrototype::initialize(JS::GlobalObject& global_object)
{
    [[maybe_unused]] auto& vm = this->vm();
    [[maybe_unused]] u8 default_attributes = JS::Attribute::Enumerable | JS::Attribute::Configurable | JS::Attribute::Writable;


    define_native_accessor("cx", cx_getter, nullptr, default_attributes);

    define_native_accessor("cy", cy_getter, nullptr, default_attributes);

    define_native_accessor("r", r_getter, nullptr, default_attributes);

    Object::initialize(global_object);
}

static JS::ThrowCompletionOr<SVG::SVGCircleElement*> impl_from(JS::VM& vm, JS::GlobalObject& global_object)
{
    ...
    if (!is<SVGCircleElementWrapper>(this_object))
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::NotAnObjectOfType, "SVG::SVGCircleElement");

    return &static_cast<SVGCircleElementWrapper*>(this_object)->impl();
}

JS_DEFINE_NATIVE_FUNCTION(SVGCircleElementPrototype::cx_getter)
{
    auto* impl = TRY(impl_from(vm, global_object));

    auto retval = TRY(throw_dom_exception_if_needed(global_object, [&] { return impl->cx(); }));

    return wrap(global_object, const_cast<SVGAnimatedLength&>(*retval));

}

JS_DEFINE_NATIVE_FUNCTION(SVGCircleElementPrototype::cy_getter)
{
    auto* impl = TRY(impl_from(vm, global_object));

    auto retval = TRY(throw_dom_exception_if_needed(global_object, [&] { return impl->cy(); }));

    return wrap(global_object, const_cast<SVGAnimatedLength&>(*retval));

}

JS_DEFINE_NATIVE_FUNCTION(SVGCircleElementPrototype::r_getter)
{
    auto* impl = TRY(impl_from(vm, global_object));

    auto retval = TRY(throw_dom_exception_if_needed(global_object, [&] { return impl->r(); }));
