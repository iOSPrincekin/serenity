...
NodeWrapper* NodeWrapper::create(JS::GlobalObject& global_object, DOM::Node& impl)
{
    return global_object.heap().allocate<NodeWrapper>(global_object, global_object, impl);
}


NodeWrapper::NodeWrapper(JS::GlobalObject& global_object, DOM::Node& impl)
    : EventTargetWrapper(global_object, impl)
{
    set_prototype(&static_cast<WindowObject&>(global_object).ensure_web_prototype<NodePrototype>("Node"));
}

void NodeWrapper::initialize(JS::GlobalObject& global_object)
{
    EventTargetWrapper::initialize(global_object);

    auto& vm = global_object.vm();
    define_direct_property(*vm.well_known_symbol_to_string_tag(), JS::js_string(vm, "Node"), JS::Attribute::Configurable);
}
