...
TreeWalkerWrapper* TreeWalkerWrapper::create(JS::GlobalObject& global_object, DOM::TreeWalker& impl)
{
    return global_object.heap().allocate<TreeWalkerWrapper>(global_object, global_object, impl);
}


TreeWalkerWrapper::TreeWalkerWrapper(JS::GlobalObject& global_object, DOM::TreeWalker& impl)
    : Wrapper(static_cast<WindowObject&>(global_object).ensure_web_prototype<TreeWalkerPrototype>("TreeWalker"))
    , m_impl(impl)
{
}

void TreeWalkerWrapper::initialize(JS::GlobalObject& global_object)
{
    Wrapper::initialize(global_object);

    auto& vm = global_object.vm();
    define_direct_property(*vm.well_known_symbol_to_string_tag(), JS::js_string(vm, "TreeWalker"), JS::Attribute::Configurable);
}

TreeWalkerWrapper::~TreeWalkerWrapper()
{
}

TreeWalkerWrapper* wrap(JS::GlobalObject& global_object, DOM::TreeWalker& impl)
{
    return static_cast<TreeWalkerWrapper*>(wrap_impl(global_object, impl));
}
