...
HTMLSelectElementWrapper* HTMLSelectElementWrapper::create(JS::GlobalObject& global_object, HTML::HTMLSelectElement& impl)
{
    return global_object.heap().allocate<HTMLSelectElementWrapper>(global_object, global_object, impl);
}


HTMLSelectElementWrapper::HTMLSelectElementWrapper(JS::GlobalObject& global_object, HTML::HTMLSelectElement& impl)
    : HTMLElementWrapper(global_object, impl)
{
    set_prototype(&static_cast<WindowObject&>(global_object).ensure_web_prototype<HTMLSelectElementPrototype>("HTMLSelectElement"));
}

void HTMLSelectElementWrapper::initialize(JS::GlobalObject& global_object)
{
    HTMLElementWrapper::initialize(global_object);

    auto& vm = global_object.vm();
    define_direct_property(*vm.well_known_symbol_to_string_tag(), JS::js_string(vm, "HTMLSelectElement"), JS::Attribute::Configurable);
}

HTMLSelectElementWrapper::~HTMLSelectElementWrapper()
{
}
