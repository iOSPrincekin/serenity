...
TextWrapper* TextWrapper::create(JS::GlobalObject& global_object, DOM::Text& impl)
{
    return global_object.heap().allocate<TextWrapper>(global_object, global_object, impl);
}


TextWrapper::TextWrapper(JS::GlobalObject& global_object, DOM::Text& impl)
    : CharacterDataWrapper(global_object, impl)
{
    set_prototype(&static_cast<WindowObject&>(global_object).ensure_web_prototype<TextPrototype>("Text"));
}

void TextWrapper::initialize(JS::GlobalObject& global_object)
{
    CharacterDataWrapper::initialize(global_object);

    auto& vm = global_object.vm();
    define_direct_property(*vm.well_known_symbol_to_string_tag(), JS::js_string(vm, "Text"), JS::Attribute::Configurable);
}
