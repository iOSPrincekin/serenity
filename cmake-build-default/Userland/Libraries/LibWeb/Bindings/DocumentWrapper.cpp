...
DocumentWrapper* DocumentWrapper::create(JS::GlobalObject& global_object, DOM::Document& impl)
{
    return global_object.heap().allocate<DocumentWrapper>(global_object, global_object, impl);
}


DocumentWrapper::DocumentWrapper(JS::GlobalObject& global_object, DOM::Document& impl)
    : NodeWrapper(global_object, impl)
{
    set_prototype(&static_cast<WindowObject&>(global_object).ensure_web_prototype<DocumentPrototype>("Document"));
}

void DocumentWrapper::initialize(JS::GlobalObject& global_object)
{
    NodeWrapper::initialize(global_object);

    auto& vm = global_object.vm();
    define_direct_property(*vm.well_known_symbol_to_string_tag(), JS::js_string(vm, "Document"), JS::Attribute::Configurable);
}
