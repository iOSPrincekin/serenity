...
SVGElementWrapper* SVGElementWrapper::create(JS::GlobalObject& global_object, SVG::SVGElement& impl)
{
    return global_object.heap().allocate<SVGElementWrapper>(global_object, global_object, impl);
}


SVGElementWrapper::SVGElementWrapper(JS::GlobalObject& global_object, SVG::SVGElement& impl)
    : ElementWrapper(global_object, impl)
{
    set_prototype(&static_cast<WindowObject&>(global_object).ensure_web_prototype<SVGElementPrototype>("SVGElement"));
}

void SVGElementWrapper::initialize(JS::GlobalObject& global_object)
{
    ElementWrapper::initialize(global_object);

    auto& vm = global_object.vm();
    define_direct_property(*vm.well_known_symbol_to_string_tag(), JS::js_string(vm, "SVGElement"), JS::Attribute::Configurable);
}

SVGElementWrapper::~SVGElementWrapper()
{
}
