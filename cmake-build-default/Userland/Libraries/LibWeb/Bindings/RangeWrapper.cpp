...
RangeWrapper* RangeWrapper::create(JS::GlobalObject& global_object, DOM::Range& impl)
{
    return global_object.heap().allocate<RangeWrapper>(global_object, global_object, impl);
}
...
{
    set_prototype(&static_cast<WindowObject&>(global_object).ensure_web_prototype<RangePrototype>("Range"));
}

void RangeWrapper::initialize(JS::GlobalObject& global_object)
{
    AbstractRangeWrapper::initialize(global_object);

    auto& vm = global_object.vm();
    define_direct_property(*vm.well_known_symbol_to_string_tag(), JS::js_string(vm, "Range"), JS::Attribute::Configurable);
}
