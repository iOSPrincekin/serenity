...
NodeIteratorWrapper* NodeIteratorWrapper::create(JS::GlobalObject& global_object, DOM::NodeIterator& impl)
{
    return global_object.heap().allocate<NodeIteratorWrapper>(global_object, global_object, impl);
}


NodeIteratorWrapper::NodeIteratorWrapper(JS::GlobalObject& global_object, DOM::NodeIterator& impl)
    : Wrapper(static_cast<WindowObject&>(global_object).ensure_web_prototype<NodeIteratorPrototype>("NodeIterator"))
    , m_impl(impl)
{
}

void NodeIteratorWrapper::initialize(JS::GlobalObject& global_object)
{
    Wrapper::initialize(global_object);

    auto& vm = global_object.vm();
    define_direct_property(*vm.well_known_symbol_to_string_tag(), JS::js_string(vm, "NodeIterator"), JS::Attribute::Configurable);
}

NodeIteratorWrapper::~NodeIteratorWrapper()
{
}

NodeIteratorWrapper* wrap(JS::GlobalObject& global_object, DOM::NodeIterator& impl)
{
    return static_cast<NodeIteratorWrapper*>(wrap_impl(global_object, impl));
}
