...
HTMLFormElementWrapper* HTMLFormElementWrapper::create(JS::GlobalObject& global_object, HTML::HTMLFormElement& impl)
{
    return global_object.heap().allocate<HTMLFormElementWrapper>(global_object, global_object, impl);
}


HTMLFormElementWrapper::HTMLFormElementWrapper(JS::GlobalObject& global_object, HTML::HTMLFormElement& impl)
    : HTMLElementWrapper(global_object, impl)
{
    set_prototype(&static_cast<WindowObject&>(global_object).ensure_web_prototype<HTMLFormElementPrototype>("HTMLFormElement"));
}

void HTMLFormElementWrapper::initialize(JS::GlobalObject& global_object)
{
    HTMLElementWrapper::initialize(global_object);

    auto& vm = global_object.vm();
    define_direct_property(*vm.well_known_symbol_to_string_tag(), JS::js_string(vm, "HTMLFormElement"), JS::Attribute::Configurable);
}

HTMLFormElementWrapper::~HTMLFormElementWrapper()
{
}
