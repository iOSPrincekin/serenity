...
DOMParserPrototype::DOMParserPrototype([[maybe_unused]] JS::GlobalObject& global_object)
    : Object(*global_object.object_prototype())

{
}

DOMParserPrototype::~DOMParserPrototype()
{
}

void DOMParserPrototype::initialize(JS::GlobalObject& global_object)
{
    [[maybe_unused]] auto& vm = this->vm();
    [[maybe_unused]] u8 default_attributes = JS::Attribute::Enumerable | JS::Attribute::Configurable | JS::Attribute::Writable;


    define_native_function("parseFromString", parse_from_string, 2, default_attributes);

    Object::initialize(global_object);
}

static JS::ThrowCompletionOr<HTML::DOMParser*> impl_from(JS::VM& vm, JS::GlobalObject& global_object)
{
    ...
    if (!is<DOMParserWrapper>(this_object))
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::NotAnObjectOfType, "HTML::DOMParser");

    return &static_cast<DOMParserWrapper*>(this_object)->impl();
}

JS_DEFINE_NATIVE_FUNCTION(DOMParserPrototype::parse_from_string)
{

    auto* impl = TRY(impl_from(vm, global_object));

    if (vm.argument_count() < 2)
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::BadArgCountMany, "parseFromString", "2");
    ...
    if (arg0.is_null() && false) {
        ...
        string = TRY(arg0.to_string(global_object));
    }

    auto arg1 = vm.argument(1);

    DOMParserSupportedType type { DOMParserSupportedType::Text_Html };
    auto arg1_string = TRY(arg1.to_string(global_object));

    if (arg1_string == "application/xhtml+xml"sv)
        type = DOMParserSupportedType::Application_Xhtml_Xml;

    else if (arg1_string == "text/xml"sv)
        type = DOMParserSupportedType::Text_Xml;

    else if (arg1_string == "image/svg+xml"sv)
        type = DOMParserSupportedType::Image_Svg_Xml;

    else if (arg1_string == "application/xml"sv)
        type = DOMParserSupportedType::Application_Xml;

    else if (arg1_string == "text/html"sv)
        type = DOMParserSupportedType::Text_Html;

    else 
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::InvalidEnumerationValue, arg1_string, "DOMParserSupportedType");

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(global_object, [&] { return impl->parse_from_string(string, type); }));
