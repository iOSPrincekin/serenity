...
FocusEventPrototype::FocusEventPrototype([[maybe_unused]] JS::GlobalObject& global_object)
    : Object(static_cast<WindowObject&>(global_object).ensure_web_prototype<UIEventPrototype>("UIEvent"))

{
}

FocusEventPrototype::~FocusEventPrototype()
{
}

void FocusEventPrototype::initialize(JS::GlobalObject& global_object)
{
    [[maybe_unused]] auto& vm = this->vm();
    [[maybe_unused]] u8 default_attributes = JS::Attribute::Enumerable | JS::Attribute::Configurable | JS::Attribute::Writable;


    define_native_accessor("relatedTarget", related_target_getter, nullptr, default_attributes);

    Object::initialize(global_object);
}

static JS::ThrowCompletionOr<UIEvents::FocusEvent*> impl_from(JS::VM& vm, JS::GlobalObject& global_object)
{
    ...
    if (!is<FocusEventWrapper>(this_object))
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::NotAnObjectOfType, "UIEvents::FocusEvent");

    return &static_cast<FocusEventWrapper*>(this_object)->impl();
}

JS_DEFINE_NATIVE_FUNCTION(FocusEventPrototype::related_target_getter)
{
    auto* impl = TRY(impl_from(vm, global_object));

    auto retval = TRY(throw_dom_exception_if_needed(global_object, [&] { return impl->related_target(); }));
