...
HTMLInputElementWrapper* HTMLInputElementWrapper::create(JS::GlobalObject& global_object, HTML::HTMLInputElement& impl)
{
    return global_object.heap().allocate<HTMLInputElementWrapper>(global_object, global_object, impl);
}


HTMLInputElementWrapper::HTMLInputElementWrapper(JS::GlobalObject& global_object, HTML::HTMLInputElement& impl)
    : HTMLElementWrapper(global_object, impl)
{
    set_prototype(&static_cast<WindowObject&>(global_object).ensure_web_prototype<HTMLInputElementPrototype>("HTMLInputElement"));
}

void HTMLInputElementWrapper::initialize(JS::GlobalObject& global_object)
{
    HTMLElementWrapper::initialize(global_object);

    auto& vm = global_object.vm();
    define_direct_property(*vm.well_known_symbol_to_string_tag(), JS::js_string(vm, "HTMLInputElement"), JS::Attribute::Configurable);
}

HTMLInputElementWrapper::~HTMLInputElementWrapper()
{
}
