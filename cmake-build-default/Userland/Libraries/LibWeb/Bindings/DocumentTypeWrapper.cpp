...
DocumentTypeWrapper* DocumentTypeWrapper::create(JS::GlobalObject& global_object, DOM::DocumentType& impl)
{
    return global_object.heap().allocate<DocumentTypeWrapper>(global_object, global_object, impl);
}


DocumentTypeWrapper::DocumentTypeWrapper(JS::GlobalObject& global_object, DOM::DocumentType& impl)
    : NodeWrapper(global_object, impl)
{
    set_prototype(&static_cast<WindowObject&>(global_object).ensure_web_prototype<DocumentTypePrototype>("DocumentType"));
}

void DocumentTypeWrapper::initialize(JS::GlobalObject& global_object)
{
    NodeWrapper::initialize(global_object);

    auto& vm = global_object.vm();
    define_direct_property(*vm.well_known_symbol_to_string_tag(), JS::js_string(vm, "DocumentType"), JS::Attribute::Configurable);
}

DocumentTypeWrapper::~DocumentTypeWrapper()
{
}
