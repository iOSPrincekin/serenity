...
DOMImplementationPrototype::DOMImplementationPrototype([[maybe_unused]] JS::GlobalObject& global_object)
    : Object(*global_object.object_prototype())

{
}

DOMImplementationPrototype::~DOMImplementationPrototype()
{
}

void DOMImplementationPrototype::initialize(JS::GlobalObject& global_object)
{
    [[maybe_unused]] auto& vm = this->vm();
    [[maybe_unused]] u8 default_attributes = JS::Attribute::Enumerable | JS::Attribute::Configurable | JS::Attribute::Writable;


    define_native_function("createDocumentType", create_document_type, 3, default_attributes);

    define_native_function("createDocument", create_document, 2, default_attributes);

    define_native_function("hasFeature", has_feature, 0, default_attributes);

    define_native_function("createHTMLDocument", create_html_document, 0, default_attributes);

    Object::initialize(global_object);
}

static JS::ThrowCompletionOr<DOM::DOMImplementation*> impl_from(JS::VM& vm, JS::GlobalObject& global_object)
{
    ...
    if (!is<DOMImplementationWrapper>(this_object))
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::NotAnObjectOfType, "DOM::DOMImplementation");

    return &static_cast<DOMImplementationWrapper*>(this_object)->impl();
}
...
JS_DEFINE_NATIVE_FUNCTION(DOMImplementationPrototype::create_document)
{
    ...
    if (vm.argument_count() < 2)
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::BadArgCountMany, "createDocument", "2");
    ...
    if (!arg2.is_nullish()) {
        if (!arg2.is_object() || !is<DocumentTypeWrapper>(arg2.as_object()))
            return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::NotAnObjectOfType, "DocumentType");

        doctype = &static_cast<DocumentTypeWrapper&>(arg2.as_object()).impl();
    }

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(global_object, [&] { return impl->create_document(namespace_, qualified_name, doctype); }));
...
JS_DEFINE_NATIVE_FUNCTION(DOMImplementationPrototype::create_html_document)
{
    ...
    if (!arg0.is_undefined()) {
        if (arg0.is_null() && false)
            ...
            title = TRY(arg0.to_string(global_object));
    }

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(global_object, [&] { return impl->create_html_document(title); }));
...
JS_DEFINE_NATIVE_FUNCTION(DOMImplementationPrototype::create_document_type)
{
    ...
    if (vm.argument_count() < 3)
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::BadArgCountMany, "createDocumentType", "3");
    ...
    if (arg1.is_null() && false) {
        ...
        public_id = TRY(arg1.to_string(global_object));
    }
    ...
    if (arg2.is_null() && false) {
        ...
        system_id = TRY(arg2.to_string(global_object));
    }

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(global_object, [&] { return impl->create_document_type(qualified_name, public_id, system_id); }));
...
JS_DEFINE_NATIVE_FUNCTION(DOMImplementationPrototype::has_feature)
{
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(global_object, [&] { return impl->has_feature(); }));
