...
#if __has_include(<LibWeb/Bindings/SVGGeometryElementPrototype.h>)
...
SVGLineElementPrototype::SVGLineElementPrototype([[maybe_unused]] JS::GlobalObject& global_object)
    : Object(static_cast<WindowObject&>(global_object).ensure_web_prototype<SVGGeometryElementPrototype>("SVGGeometryElement"))

{
}

SVGLineElementPrototype::~SVGLineElementPrototype()
{
}

void SVGLineElementPrototype::initialize(JS::GlobalObject& global_object)
{
    [[maybe_unused]] auto& vm = this->vm();
    [[maybe_unused]] u8 default_attributes = JS::Attribute::Enumerable | JS::Attribute::Configurable | JS::Attribute::Writable;


    define_native_accessor("x1", x1_getter, nullptr, default_attributes);

    define_native_accessor("y1", y1_getter, nullptr, default_attributes);

    define_native_accessor("x2", x2_getter, nullptr, default_attributes);

    define_native_accessor("y2", y2_getter, nullptr, default_attributes);

    Object::initialize(global_object);
}

static JS::ThrowCompletionOr<SVG::SVGLineElement*> impl_from(JS::VM& vm, JS::GlobalObject& global_object)
{
    ...
    if (!is<SVGLineElementWrapper>(this_object))
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::NotAnObjectOfType, "SVG::SVGLineElement");

    return &static_cast<SVGLineElementWrapper*>(this_object)->impl();
}

JS_DEFINE_NATIVE_FUNCTION(SVGLineElementPrototype::x1_getter)
{
    auto* impl = TRY(impl_from(vm, global_object));

    auto retval = TRY(throw_dom_exception_if_needed(global_object, [&] { return impl->x1(); }));

    return wrap(global_object, const_cast<SVGAnimatedLength&>(*retval));

}

JS_DEFINE_NATIVE_FUNCTION(SVGLineElementPrototype::y1_getter)
{
    auto* impl = TRY(impl_from(vm, global_object));

    auto retval = TRY(throw_dom_exception_if_needed(global_object, [&] { return impl->y1(); }));

    return wrap(global_object, const_cast<SVGAnimatedLength&>(*retval));

}

JS_DEFINE_NATIVE_FUNCTION(SVGLineElementPrototype::x2_getter)
{
    auto* impl = TRY(impl_from(vm, global_object));

    auto retval = TRY(throw_dom_exception_if_needed(global_object, [&] { return impl->x2(); }));

    return wrap(global_object, const_cast<SVGAnimatedLength&>(*retval));

}

JS_DEFINE_NATIVE_FUNCTION(SVGLineElementPrototype::y2_getter)
{
    auto* impl = TRY(impl_from(vm, global_object));

    auto retval = TRY(throw_dom_exception_if_needed(global_object, [&] { return impl->y2(); }));
