...
ElementWrapper* ElementWrapper::create(JS::GlobalObject& global_object, DOM::Element& impl)
{
    return global_object.heap().allocate<ElementWrapper>(global_object, global_object, impl);
}


ElementWrapper::ElementWrapper(JS::GlobalObject& global_object, DOM::Element& impl)
    : NodeWrapper(global_object, impl)
{
    set_prototype(&static_cast<WindowObject&>(global_object).ensure_web_prototype<ElementPrototype>("Element"));
}

void ElementWrapper::initialize(JS::GlobalObject& global_object)
{
    NodeWrapper::initialize(global_object);

    auto& vm = global_object.vm();
    define_direct_property(*vm.well_known_symbol_to_string_tag(), JS::js_string(vm, "Element"), JS::Attribute::Configurable);
}
