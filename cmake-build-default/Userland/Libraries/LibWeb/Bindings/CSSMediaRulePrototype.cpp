...
#if __has_include(<LibWeb/Bindings/CSSConditionRulePrototype.h>)
...
CSSMediaRulePrototype::CSSMediaRulePrototype([[maybe_unused]] JS::GlobalObject& global_object)
    : Object(static_cast<WindowObject&>(global_object).ensure_web_prototype<CSSConditionRulePrototype>("CSSConditionRule"))

{
}

CSSMediaRulePrototype::~CSSMediaRulePrototype()
{
}

void CSSMediaRulePrototype::initialize(JS::GlobalObject& global_object)
{
...
static JS::ThrowCompletionOr<CSS::CSSMediaRule*> impl_from(JS::VM& vm, JS::GlobalObject& global_object)
{
    ...
    if (!is<CSSMediaRuleWrapper>(this_object))
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::NotAnObjectOfType, "CSS::CSSMediaRule");

    return &static_cast<CSSMediaRuleWrapper*>(this_object)->impl();
}

JS_DEFINE_NATIVE_FUNCTION(CSSMediaRulePrototype::media_getter)
{
    auto* impl = TRY(impl_from(vm, global_object));

    auto retval = TRY(throw_dom_exception_if_needed(global_object, [&] { return impl->media(); }));

    return wrap(global_object, const_cast<MediaList&>(*retval));
