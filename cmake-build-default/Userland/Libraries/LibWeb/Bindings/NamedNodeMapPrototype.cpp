...
NamedNodeMapPrototype::NamedNodeMapPrototype([[maybe_unused]] JS::GlobalObject& global_object)
...
void NamedNodeMapPrototype::initialize(JS::GlobalObject& global_object)
{
    ...
    define_native_function("removeNamedItem", remove_named_item, 1, default_attributes);

    define_native_function("getNamedItem", get_named_item, 1, default_attributes);

    define_native_function("setNamedItem", set_named_item, 1, default_attributes);
...
static JS::ThrowCompletionOr<DOM::NamedNodeMap*> impl_from(JS::VM& vm, JS::GlobalObject& global_object)
{
    ...
    if (!is<NamedNodeMapWrapper>(this_object))
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::NotAnObjectOfType, "DOM::NamedNodeMap");

    return &static_cast<NamedNodeMapWrapper*>(this_object)->impl();
}
...
JS_DEFINE_NATIVE_FUNCTION(NamedNodeMapPrototype::get_named_item)
{
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::BadArgCountOne, "getNamedItem");
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(global_object, [&] { return impl->get_named_item(qualified_name); }));
...
JS_DEFINE_NATIVE_FUNCTION(NamedNodeMapPrototype::set_named_item)
{
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::BadArgCountOne, "setNamedItem");

    auto arg0 = vm.argument(0);

    if (!arg0.is_object() || !is<AttributeWrapper>(arg0.as_object()))
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::NotAnObjectOfType, "Attribute");

    auto& attr = static_cast<AttributeWrapper&>(arg0.as_object()).impl();

    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(global_object, [&] { return impl->set_named_item(attr); }));
...
JS_DEFINE_NATIVE_FUNCTION(NamedNodeMapPrototype::remove_named_item)
{
    ...
    if (vm.argument_count() < 1)
        return vm.throw_completion<JS::TypeError>(global_object, JS::ErrorType::BadArgCountOne, "removeNamedItem");
    ...
    [[maybe_unused]] auto retval = TRY(throw_dom_exception_if_needed(global_object, [&] { return impl->remove_named_item(qualified_name); }));
