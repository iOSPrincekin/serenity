...
ShadowRootWrapper* ShadowRootWrapper::create(JS::GlobalObject& global_object, DOM::ShadowRoot& impl)
{
    return global_object.heap().allocate<ShadowRootWrapper>(global_object, global_object, impl);
}


ShadowRootWrapper::ShadowRootWrapper(JS::GlobalObject& global_object, DOM::ShadowRoot& impl)
    : DocumentFragmentWrapper(global_object, impl)
{
    set_prototype(&static_cast<WindowObject&>(global_object).ensure_web_prototype<ShadowRootPrototype>("ShadowRoot"));
}

void ShadowRootWrapper::initialize(JS::GlobalObject& global_object)
{
    DocumentFragmentWrapper::initialize(global_object);

    auto& vm = global_object.vm();
    define_direct_property(*vm.well_known_symbol_to_string_tag(), JS::js_string(vm, "ShadowRoot"), JS::Attribute::Configurable);
}

ShadowRootWrapper::~ShadowRootWrapper()
{
}

ShadowRootWrapper* wrap(JS::GlobalObject& global_object, DOM::ShadowRoot& impl)
{
    return static_cast<ShadowRootWrapper*>(wrap_impl(global_object, impl));
}
